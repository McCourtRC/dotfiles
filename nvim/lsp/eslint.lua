...
return {
  ...
  -- Neovim 0.11 reads this to find the project root
  root_markers = {
    "eslint.config.js",
    "eslint.config.cjs",
    ".eslintrc",
  ...
  settings = {
    -- Make sure ESLint gets a string here (workaround for a server bug)
    nodePath = "",

    -- Tell the server which ESLint config style you use:
    --   * true  -> ESLint v9 “flat” config (eslint.config.*)
    --   * false -> legacy .eslintrc*
    experimental = { useFlatConfig = true },  -- set to false if you use .eslintrc

    -- Helps in monorepos/workspaces
    workingDirectory = { mode = "auto" },

    -- (Harmless no-ops, but avoid another bug path in some setups)
    problems = {},
    rulesCustomizations = {},
  },
}
